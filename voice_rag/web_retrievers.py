...
# All network errors are caught and returned as empty lists (safe failure).
# """
#
...
#
# # Configure module-level logger (uses existing logging config if any)
# logger = logging.getLogger(__name__)
#
# # Default timeouts
# _REQUEST_TIMEOUT = 8  # seconds
# # Required for Wikipedia (and recommended for arXiv)
#
# _DEFAULT_HEADERS = {
#     "User-Agent": "LocalVoiceRAG/1.0 (https://github.com/example; contact: local@localhost)"
# }
#
#
# def _safe_get(url: str, params: dict = None, headers: dict = None, timeout: int = _REQUEST_TIMEOUT):
#     try:
#         final_headers = _DEFAULT_HEADERS.copy()
#         if headers:
#             final_headers.update(headers)
#         r = requests.get(url, params=params, headers=final_headers, timeout=timeout)
#         r.raise_for_status()
#         return r
#     except Exception as e:
#         logger.warning(f"web_retrievers: GET {url} failed: {e}")
#         return None
#
#
#
# def retrieve_wikipedia(query: str, max_results: int = 3) -> List[Dict]:
#     results = []
#     try:
#         if not query:
#             return results
...
#             }
#
#             results.append({"text": extract, "meta": meta})
#
#     except Exception as e:
#         logger.warning(f"retrieve_wikipedia failed: {e}")
#
#     return results
#
# def retrieve_arxiv(query: str, max_results: int = 3) -> List[Dict]:
#     import html
#     results = []
#     seen_ids = set()
#
...
#                 article_id = id_elem.text.strip() if id_elem is not None else None
...
#                 summary_raw = summary_elem.text if summary_elem is not None else ""
#
#                 # Clean + unescape + normalize whitespace
#                 summary = " ".join(html.unescape(summary_raw).split())
#
#                 # Remove arXiv echo/noise
#                 if title.lower().startswith("arxiv query"):
#                     continue
#
#                 # Remove truly empty summaries only
#                 if len(summary) < 20:
#                     continue
#
...
#
#             except Exception as e:
#                 logger.warning(f"web_retrievers: arXiv entry parse failed: {e}")
#                 continue
#
#     except Exception as e:
...
# #             return results
# #
# #         # extract signal from natural language
# #         tokens = [t.strip(".,!?").lower() for t in query.split()]
# #         tokens = [t for t in tokens if len(t) > 3]  # remove stopwords
# #         if not tokens:
# #             return results
# #
# #         q = " OR ".join(f"all:{t}" for t in tokens)
...
# #
# #                 # ---- FILTERS TO REMOVE JUNK ----
# #
# #                 # 1. Remove arXiv's synthetic query-echo result
# #                 if title.lower().startswith("arxiv query"):
# #                     continue
# #
# #                 summary = summary_elem.text.strip() if summary_elem is not None and summary_elem.text else ""
# #
# #                 # 2. Remove entries with no substantial summary
# #                 if len(summary) < 80:
# #                     continue
# #
# #                 # --------------------------------
...
#     """
#     Retrieve web results from Tavily Search API.
#     Returns: list of {"text": ..., "meta": {...}} dictionaries.
#     """
#     results = []
#
#     if not query:
...
#     if not api_key:
#         logger.info("web_retrievers: Tavily API key not found; skipping Tavily retrieval.")
#         return results
#
#     # 2. Correct Tavily endpoint (REMOVE /v1/)
#     # The standard endpoint is simply /search
...
#
#     # 4. Correct payload
#     # Tavily prefers the api_key inside the body, not the header.
#     payload = {
#         "api_key": api_key,
#         "query": query,
#         "max_results": max_results,
...
#     try:
#         r = requests.post(
#             tavily_endpoint, json=payload, headers=headers, timeout=_REQUEST_TIMEOUT
#         )
#         r.raise_for_status()
#         data = r.json()
#     except Exception as e:
#         logger.warning(f"web_retrievers: Tavily request failed: {e}")
#         return results
#
#     # 5. Process results
#     # Tavily response structure: {'results': [{'title': '...', 'url': '...', 'content': '...'}]}
...
#
#         # Extract metadata
#         # We explicitly grab url and title as they are critical for citations
#         meta = {
#             "source": "tavily",
#             "retrieved_at": time.time(),
#             "url": it.get("url"),
#             "title": it.get("title"),
#             "score": it.get("score") # Relevance score if available
#         }
#
#         # Merge any existing metadata from the API that we didn't explicitly grab
#         existing_meta = it.get("metadata", {})
...
# ) -> List[Dict]:
#     """
#     Convenience function to run the enabled web retrievers and return a concatenated list of
#     {"text": ..., "meta": {...}}. Failures become empty lists.
#     """
#     aggregated = []
#     try:
#         if use_wiki:
#             try:
#                 wiki_docs = retrieve_wikipedia(query, max_per_source)
#                 aggregated.extend(wiki_docs)
#             except Exception as e:
#                 logger.warning(f"web_retrievers: retrieve_wikipedia raised: {e}")
#
#         if use_arxiv:
#             try:
#                 arxiv_docs = retrieve_arxiv(query, max_per_source)
#                 aggregated.extend(arxiv_docs)
#             except Exception as e:
#                 logger.warning(f"web_retrievers: retrieve_arxiv raised: {e}")
#
#         if use_tavily:
#             try:
#                 tavily_docs = retrieve_tavily(query, max_per_source)
#                 aggregated.extend(tavily_docs)
#             except Exception as e:
#                 logger.warning(f"web_retrievers: retrieve_tavily raised: {e}")
#     except Exception as e:
#         logger.warning(f"web_retrievers: retrieve_all failed: {e}")
#
#     return aggregated


"""
web_retrievers.py (patched)
...
- Defensive network calls with timeouts and retries (configurable)
- Clear provenance fields (source, url, title, published, retrieved_at)
- Robust parsing for Wikipedia and arXiv XML/JSON responses
- Tavily integration that respects env var presence and defensively parses results
- Structured logging and no side effects on import
"""
"""
web_retrievers.py (patched, fixed)
